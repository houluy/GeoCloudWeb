...
<div id="layout" @mousemove="moveEvent" @click="moveEvent">
  <HeaderDZY @getisLogin="getisLogin" ref="cyHeader" v-if="!this.navHide" />
  <NavDZY :isLoginD = 'isLoginD' :activeMenu="activeMenu" v-show="!this.navHide" />
  <cy-main />
  <cy-map v-if="mapFlag" />
</div>
</template>
...
import HeaderDZY from './components/Header.vue'
import NavDZY from './components/Nav.vue'
...
export default {
  ...
  methods: {
    ...
    getTokenData () {
      ...
      // //alert(localStorage.getItem('dzytoken').length)
      ...
      */
      ...
      if (!localStorage.getItem('dzytoken')&&!param.code) {
        //alert("localStorage.getItem('dzytoken')&&param.code:"+localStorage.getItem('dzytoken'))
        //alert("param.code:"+param.code)
        ...
        //alert("param.codeparam.codeparam.code:"+param.code)
        /* token不存在且授权码存在
        ...
        */
        http.getTokenData({ code: param.code }).then(res => {
          if (res.data) {
            ...
            localStorage.setItem('dzytoken', res.data)
