      ...
      if (this.currentTime - this.lastTime > this.timeOut) {
        ...
        this.$alert("检测到您长时间未操作页面,请重新登录!", "温馨提示", {
          ...
          callback: (action) => {
            ...
            http.logout({ loginName: logName }).then(
              (res) => {
                http
                  .deleteTokenByUserName({ loginName: logName })
                  .then((res) => {
                ...
                window.location.href =
                  "https://geocloudsso.cgs.gov.cn/ssoserver/login/loginout";
                this.removeCookie();
              },
              (resp) => {
              ...
              http.getUserInfo().then((response) => {
                ...
                localStorage.setItem(
                  "userName",
                  JSON.parse(data.principal).userName
                );
                localStorage.setItem("id", JSON.parse(data.principal).id);
                sessionStorage.setItem(
                  "userId",
                  JSON.parse(data.principal).userId
                );
                localStorage.setItem(
                  "userId",
                  JSON.parse(data.principal).userId
                ...
                localStorage.setItem(
                  "roleIds",
                  JSON.parse(data.principal).roleIds
                );
                ...
                if (localStorage.getItem("roleIds").indexOf("管理员") > -1) {
                  this.$router.push({
                    path: "/manage/person/orderManage_stayAudit",
                  });
                }
              });
          },
