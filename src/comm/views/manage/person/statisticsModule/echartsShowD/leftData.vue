...
<div>
  <div style="width: 49%; float:left; margin-right: 2%">
    <div>
      <div :class="{activeLeft: index === 0 }" class="con_gl" v-for="(item, index) in leftDataTop" :key="index">
        <img style="position: relative;
    top: -5px;float:left;" :src="item.icon" alt="">
  ...
  <div style="width: 49%; float:left;">
    <div>
      <div :class="{activeLeft: index === 0 }" class="con_gl1" v-for="(item, index) in leftDataTop1" :key="index">
        <img style="position: relative;
    top: -2.5px;float:left;width: 24px" :src="item.icon" alt="">
        <div style="float:left; width: 73%;">
          <span class="span_one1">{{item.name}}</span>
          <span class="span_two1">{{item.value}} <span v-if="item.type !== undefined" class="span_two_type">{{item.type}}</span></span>
        </div>
      </div>
  ...
  <div style="width: 49%; float:left;margin-top: 6px">
    <div>
      <div :class="{activeLeft: index === 0 }" class="con_gl1" v-for="(item, index) in leftDataTop2" :key="index">
        <img style="position: relative;
    top: -2.5px;float:left;width: 24px" :src="item.icon" alt="">
        <div style="float:left; width: 73%;">
          <span class="span_one1">{{item.name}}</span>
...
export default {
  ...
  methods: {
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            htmlStr += '<span style="margin-right:5px;display:inline-block;width:10px;height:10px;border-radius:5px;background-color:' + params.color + ';"></span>' + params.name + ':' + params.data.value + '(' + params.percent + '%) ' + '<br/>'
            htmlStr += '</div>'
            return htmlStr
          }
        },
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            let htmlStr = '<div>'
            // htmlStr += params.name + ': ' + '<br/>'// x轴的名称
            htmlStr += '<span style="margin-right:2.5px;display:inline-block;width:5px;height:5px;border-radius:5px;background-color:' + params.color + ';"></span>' + params.name + ':' + params.data.value + '(' + params.percent + '%) ' + '<br/>'
            htmlStr += '</div>'
            return htmlStr
          }
        },
    ...
    excelDown () {
      if (this.leftDataTop[0].value !== '0' && this.leftDataTop[1].value !== '0' && this.leftDataTop1[0].value
        !== '0' && this.leftDataTop1[1].value !== '0' && this.leftDataTop2[0].value !== '0' && this.leftDataTop2[1].value !== '0') {
        this.downData = [
          [
            ['在线订单数', '离线订单数','订单总数','在线订单数据量', '离线订单数据量','订单总数据量','在线订单景数', '离线订单景数','订单总景数'],
            [
              this.leftDataTop[1].value, this.leftDataTop[0].value,parseInt(this.leftDataTop[1].value)+parseInt(this.leftDataTop[0].value),
              this.leftDataTop1[1].value, this.leftDataTop1[0].value,parseFloat(this.leftDataTop1[1].value)+parseFloat(this.leftDataTop1[0].value),
              this.leftDataTop2[1].value, this.leftDataTop2[0].value,parseInt(this.leftDataTop2[1].value)+parseInt(this.leftDataTop2[0].value)
            ]
          ],
          [
            // 设置A1-C1的单元格合并
    ...
    echartsLeftData: {
      handler (newval) {
       ...
       /*   let offsizeType = this.commonMZ.dataSizeFormatter(newval.offlineOrderSize)
          ...
          let allNum = parseInt(newval.onlineOrderNum) + parseInt(newval.offlineOrderNum)
          let allSceneNum = parseInt(newval.offlineOrderSceneNum) + parseInt(newval.onlineOrderSceneNum)
