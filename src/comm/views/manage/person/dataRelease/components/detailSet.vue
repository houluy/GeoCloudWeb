  ...
  export default {
    ...
    methods:{
      ...
      toolsFeatureExecl (val) {
        console.log(val.name, 'detailset')
        let param ={
          exePath: val.name,
      ...
      getDisplayFiledByNodeIdP (param) {
        http.getNodeFiledByNodeId({data: {fnodeid: parseInt(param)}}).then(res => {
      ...
      getAllTableRowByNodeIdP (param) {
        this.checkList = []
        http.getAllNodeTableRowByNodeId({data: {fnodeid: parseInt(param)}}).then(res => {
          this.checkData = []
      ...
      saveOrUpdateDisplayFiledP () {
        if (this.tableData.length > 0) {
          ...
          http.saveOrUpdateNodeFiled({data: {agrsNodeFiledList: this.tableData}}).then(res => {
            console.log(res, 'popopopopopo')
            if (res.data) {
              this.$message({
                message: '保存成功！',
                type: 'success'
    ...
    watch: {
      nodeDataIndex: {
        handler(newVal, oldVal) {
          ...
          if (newVal !== 0 && this.activeName === 'four'){
