        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <img title="反馈附件下载" style="cursor: pointer;" v-if="orderStatusIndex === 8 && scope.row.feedBackUpload" @click="openFeedBackXZ(scope.row)"  src="@/assets/img/fk_xz.png" alt="">
             ...
             <img v-if="(orderStatusIndex === 0 || scope.row.sstatus === 0)&& userProName === '管理员'" title="审核不通过" style="margin-right: 5px; cursor: pointer;" @click="handleEditBtg(2, scope.row)" width="16px" height="16px" src="@/assets/img/rejectD.png" alt="">
            ...
            <i class="infoContactd" v-if=" scope.row.sstatus === 6 && scope.row.getMethod === '线下拷贝'" >
              <el-popover
                ...
                <div v-if="userProName === '管理员'" style="display: flex; margin-bottom: 10px">
                  <span style="flex: 1;">物理地址:</span><span style="flex: 2;">{{scope.row.downlodPathFile}}</span>
                </div>
                <div style="line-height: 22px;text-align: center;color: blue;cursor:pointer;" @click="checkInfo()" >点击查看详情</div>
                <el-button slot="reference"><i class="el-icon-chat-dot-round" style="font-size: 16px; color: blue; cursor: pointer; font-weight: bold; position: relative;"></i></el-button>
              </el-popover>
            </i>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getUsernameNum () {
      if (this.usernameNum) {
        ...
        if (window.systemName === 'DZY') {
          ...
          this.loginName = localStorage.getItem('userId')
        }
      }
    },
    findOrderList () {
    ...
    audit (val, row) {
      console.log("row",row)
      if(row !== undefined && this.dialogVisible === false){
        http.addUnit({data:{
...
.FeedBack_evaluate_left_rate{
  ...
  /*.table >>> .el-table__body .el-table__row .cell div:nth-child(2) {*/
    /*width: 100% !important;*/
  /*}*/
</style>
