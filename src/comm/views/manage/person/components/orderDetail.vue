<template>
  <div class="createOrder">
    <div class="createOrder_one" v-if="!showDiv">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <div>
            ...
            <span
              >订单基本信息（{{
                orderDetlia.dataType === "1" ? "资料数据" : "产品数据"
              }}）</span
            >
          </div>
        </div>
        <div class="content">
          <basicOrderInfo
            v-if="this.orderStatusIndex === -1 || this.orderStatusIndex === 2"
            @basicOrderInfoD="basicOrderInfoD"
          ></basicOrderInfo>
          <orderNoAduti
            v-if="
      ...
      <el-card class="box-card" style="margin: 0.2rem 0">
        <div slot="header" class="clearfix">
          <div>
            <img class="spanIMG_Z" src="@/assets/img/spanIMG.png" alt="" />
            <span>订单附件</span
        ...
        <div class="content_tow">
          <div
            v-if="
              this.orderStatusIndex === 0 ||
              this.orderStatusIndex === 1 ||
              (this.orderStatusIndex === 6 && userProName !== '管理员') ||
              this.orderStatusIndex === 5 ||
              this.orderStatusIndex === 7 ||
              this.orderStatusIndex === 8
            "
          >
            <div
              class="content_UP"
              v-for="(item, index) in upDataD"
              :key="index"
              :class="{
                content_UPz_Z: index === 2 || index === 4,
                content_UP_b: index === 3,
              }"
            >
              <div style="margin-bottom: 14px">
                {{ item.label
                  ...
                  v-if="
                    (orderDetlia.getMethod === '在线下载' && index < 4) ||
                    (orderDetlia.getMethod !== '在线下载' && index < 2) ||
                    (orderDetlia.getMethod !== '在线下载' && index === 3)
                  "
                  src="@/assets/img/noetmp.png"
                  alt=""
                />
              </div>
              <div v-for="(e, index) in item.file" :key="index">
                <a
                  ...
                  @click="
                    opendown(
                      e.fDataPath,
                      e.fDataName.slice(19, e.fDataName.length)
                    )
                  "
                  >{{ e.fDataName.slice(19, e.fDataName.length) }}</a
                >
              </div>
            </div>
          </div>
          <upDataFile
            v-if="
              ...
              (this.orderStatusIndex === 6 && userProName === '管理员')
            "
            @upDataFileClick="upDataFileClick"
            :UpgetMethod="UpgetMethod"
          ></upDataFile>
        </div>
      </el-card>
      <el-card
        ...
        <div class="content content_pag">
          ...
          >
            ...
            <el-table-column label="数据状态" align="center">
              <template slot-scope="scope">
                <span
                  @click.stop="getFlogData(scope.row)"
                  :class="{ colorindexO: scope.row.sstatus === 2 }"
                  v-show="orderStatusIndex === 6 || orderStatusIndex === 1"
                  >{{ dataStutas[scope.row.sstatus] }}
                  <i v-if="scope.row.sstatus === 2" class="el-icon-search"></i
                ></span>
                <span v-if="orderStatusIndex !== 6 && orderStatusIndex !== 1"
                  >-</span
                >
              </template>
              <span></span>
          ...
          <div class="button_pag">
            <el-checkbox
              ...
              @change="changeshopTableDataAll"
              style="float: left; margin: 15px 19px 0"
              v-model="checkedAllData"
              >全 选</el-checkbox
            >
            <el-button
              style="float: left; margin: 10px 10px 0"
              @click="exportData"
              size="mini"
              type="primary"
              >导出</el-button
            ...
            <el-button
              v-if="
                userProName === '管理员' && repOrder && orderStatusIndex === 1
              "
              ...
              >重提</el-button
            >
            <span style="font-size: 13px; float: right; margin: 20px 10px 0"
              >总共 {{ this.totalNum }} 页</span
            >
            <el-pagination
              class="pagination"
      ...
      <el-card
        class="box-card mapDivDialogDec"
        style="margin: 0.2rem 0; width: 48.6%; float: left"
      >
      ...
      <div style="width: 100%; text-align: center" class="">
        ...
        <div v-if="orderStatusIndex === 0">
          <div v-if="userProName === '管理员'">
            <el-button
              @click="handleEditTg(1, orderDetlia)"
              style="margin: 0.2rem"
              type="primary"
              >审核通过</el-button
            >
            <el-button
              @click="handleEditBtg(2, orderDetlia)"
              style="margin: 0.2rem"
              type="danger"
              >驳 回</el-button
            >
        ...
        <div v-if="orderStatusIndex === 2">
          <el-button
            @click="goSubmitData(-1)"
            style="margin: 0.2rem"
            type="primary"
            >暂 存</el-button
          >
          <el-button
            @click="goSubmitData(0)"
            style="margin: 0.2rem"
            type="primary"
            >重新提交</el-button
          >
        ...
        <div v-if="orderStatusIndex === 6">
          <el-button @click="getOrderFile" style="margin: 0.2rem" type="danger"
            >下载订单附件</el-button
          >
          <el-button
            v-if="userProName === '管理员'"
            @click="goSubmitData(7, '订单状态置为完成！')"
            style="margin: 0.2rem"
            type="success"
            >完成订单</el-button
          >
          ...
          <el-button
            v-if="userProName === '管理员'"
            @click="changeState(5, orderDetlia.id)"
            style="margin: 0.2rem"
            type="warning"
            >逾期订单</el-button
          >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="small" type="primary" @click="audit(2, orderDetlia)"
          >确 定</el-button
        >
    ...
    <el-dialog
      class="elDialog_Aduti"
      :title="titleFlog"
      :visible.sync="dialogVisibleFlog"
      width="7rem"
    >
      <span>
        <div style="width: 100%">
...
export default {
  ...
  mounted() {
    this.pageSize = this.changeFixedPageSizeZ(
      document.documentElement.clientHeight * 0.52 - 42,
      42
    );
    this.heightPag = document.documentElement.clientHeight * 0.52 - 42;
    ...
    this.$nextTick(() => {
      ...
      if (orderDataID) {
        ...
        this.orderDetlia.creattime = this.commonMethod.setDateTime(
          this.orderDetlia.creattime
        );
  ...
  methods: {
    ...
    baseMapInit() {
      ...
      gisMapDiaDe.baseMapInit(
        baseMapInitParameters,
        () => {
          gisMapDiaDe.map.on(
            "moveend",
            gisMapDiaDe.checkZoomAndCenter.bind(gisMapDiaDe)
          );
        },
        "mapDivDialogDe",
        false
    ...
    handleSelectionChange(val) {
      ...
      if (
        that.checkedAllData &&
        (that.noSelect.length > 0 || that.noSelectPage.length > 0)
      ) {
    ...
    exportData() {
      if (this.checkedAllData || this.isIndeterminate) {
        let dids = [];
        this.noSelectPage = this.reduceSame(
          this.noSelectPage.concat(this.noSelect)
        );
    ...
    getDataUrlFJ() {
      http
        .agrsGetOrderOldFile({
          data: { ordername: this.orderDetlia.ordername },
        })
        .then((res) => {
          if (res.data !== "false") {
        ...
        .then((res) => {
          ...
          if (res.data.list.length > 0) {
            res.data.list.forEach((item, index) => {
              item.imageurl =
                "/mj/metaImage/getImageByTypeForAll?typeId=2&dataId=" +
                item.did +
                "&nodeId=" +
                item.nodeid;
              item.datasize = this.commonMethod.dataSizeFormatter(
                item.datasize
              );
              item.scenedate = this.commonMethod.timestampToTime(
                item.scenedate
              );
    ...
    upDataFileTow(val, string) {
      let fileArr = this.uploadFileList.taskDataCard.concat(
        this.uploadFileList.regisForm,
        this.uploadFileList.introductionLetter,
        this.uploadFileList.identityCard,
        this.uploadFileList.otherDoc
      );
    ...
    goSubmitData(val, string) {
      if (
        this.comData === 0 &&
        this.orderStatusIndex === 2 &&
        this.formLabelAlign.getMethod === this.orderDetlia.getMethod &&
        this.formLabelAlign.user === this.orderDetlia.applyuser &&
        this.formLabelAlign.phone === this.orderDetlia.applyUserPhone &&
        this.formLabelAlign.proName === this.orderDetlia.productName &&
        this.formLabelAlign.company === this.orderDetlia.applyUserUnit &&
        this.formLabelAlign.employ === this.orderDetlia.applyUserUsed &&
        this.textarea === this.orderDetlia.description
      ) {
        this.$message({
          message: "请更改信息！",
          type: "warning",
      ...
      if (
        ...
        this.formLabelAlign.getMethod === "" ||
        this.formLabelAlign.proName === "" ||
        this.formLabelAlign.employ === "" ||
    ...
    goSubmit(val, string) {
      if (val !== -1) {
        if (this.formLabelAlign.getMethod === "在线下载") {
          if (
            this.uploadFileList.taskDataCard.length === 0 ||
            this.uploadFileList.identityCard.length === 0 ||
            this.uploadFileList.introductionLetter.length === 0 ||
            this.uploadFileList.regisForm.length === 0
          ) {
            this.$message({
              message: "请上传必传项！",
              type: "warning",
...
#uploadFilesDialog ::v-deep .el-form-item__label {
  border-left: 0.1rem solid #2e6cfc;
  margin-top: 5px;
  line-height: 14px;
}
